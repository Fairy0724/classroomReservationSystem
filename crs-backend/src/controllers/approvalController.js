...
const getPendingApprovals = async (req, res) => {
  try {
    const teacherId = await resolveTeacherId(req.user.user_id);

    const [rows] = await pool.query(
      `SELECT r.*, 
            u.real_name AS applicant_name,
            c.building, c.room_num, c.dept_name,
              COALESCE(a.applicant_id, r.applicant_id) AS applicant_id
     FROM reservation r
       LEFT JOIN approval_record a ON a.reservation_id = r.reservation_id
     LEFT JOIN user u ON COALESCE(a.applicant_id, r.applicant_id) = u.user_id
...
/**
 * 教师审批记录
 * 来源：approval_record 表 + reservation 关联
 */
const getApprovalRecords = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
     ...
     LEFT JOIN user u ON COALESCE(a.applicant_id, r.applicant_id) = u.user_id
     LEFT JOIN classroom c ON r.classroom_id = c.classroom_id
     WHERE a.teacher_id = ? AND a.result IN ('通过','驳回')
 ...
 * - 已通过/已驳回：approval_record.result 统计
 */
const getApprovalStats = async (req, res) => {
  try {
    const teacherId = await resolveTeacherId(req.user.user_id);

    const hasApprovalTable = await tableExists('approval_record');
    ...
    if (hasApprovalTable) {
      const [[approvedRow]] = await pool.query(
        `SELECT COUNT(*) AS count FROM approval_record
  ...
  try {
    ...
    const [updateRes] = await connection.query(
      `UPDATE approval_record
       SET result = ?, reason = ?, approval_time = NOW()
       WHERE reservation_id = ? AND teacher_id = ? AND result = '待审批'
       LIMIT 1`,
      [result, reason || null, id, teacherId]
    );
    ...
    if (!updateRes.affectedRows) {
      await connection.query(
        `INSERT INTO approval_record (reservation_id, applicant_id, teacher_id, result, reason, approval_time)
			 VALUES (?, ?, ?, ?, ?, NOW())`,
        [id, record.applicant_id, teacherId, result, reason || null]
      );
    }
